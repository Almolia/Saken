import { Building, Calendar, Check, Clock, RefreshCw, Trash2, AlertCircle } from 'lucide-react'
...
export function AmenityBookingSection() {
  ...
  const [tab, setTab] = useState('book') // 'book' | 'my_bookings'
  ...
  async function handleCancelReservation(resId) {
    ...
    try {
      const response = await amenityApi.cancelReservation(resId)
      ...
      showToast(error.message || 'خطا در لغو رزرو', 'error')
    } finally {
      setCancelingId(null)
    }
  }

  const activeReservationsCount = myReservations.filter((r) => r.status === 'Active').length
      ...
      <div className="flex flex-col gap-4 border-b border-slate-100 p-6 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 rounded-2xl px-4 py-2 text-xs font-extrabold transition ${
              tab === 'my_bookings'
                ? 'bg-teal-600 text-white shadow-sm'
                : 'bg-slate-100 text-slate-700 hover:bg-slate-200'
            }`}
          >
            ...
            {activeReservationsCount > 0 && (
              <span className="flex h-5 w-5 items-center justify-center rounded-full bg-teal-100 text-[11px] font-black text-teal-800">
      ...
      {tab === 'my_bookings' ? (
        <div className="p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-black text-slate-900">رزروهای فعال و گذشته من</h3>
            <button
              type="button"
              onClick={() => {
                setLoadingMyReservations(true)
              ...
              className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-slate-700 transition hover:bg-slate-50 disabled:opacity-50"
            >
              <RefreshCw className={`h-3.5 w-3.5 ${loadingMyReservations ? 'animate-spin' : ''}`} />
              به‌روزرسانی
            </button>
          </div>
          ...
          {loadingMyReservations ? (
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {myReservations.map((res) => {
                ...
                const startObj = new Date(res.start_time)
                const endObj = new Date(res.end_time)
                const dateFormatted = startObj.toLocaleDateString('fa-IR')
                const timeFormatted = `${startObj.toLocaleTimeString('fa-IR', { hour: '2-digit', minute: '2-digit' })} تا ${endObj.toLocaleTimeString('fa-IR', { hour: '2-digit', minute: '2-digit' })}`
                    ...
                    <div>
                      <div className="flex items-center justify-between">
                        <span className="text-sm font-black text-slate-900">
                          {res.amenity_name || `امکان #${res.amenity}`}
                        </span>
                        <span
