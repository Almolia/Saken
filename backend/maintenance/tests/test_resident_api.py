...
class ResidentRequestAPITests(APITestCase):
    ...
    def setUp(self):
        self.user_a = User.objects.create_user(
            phone='09121111111', password='passwordA',
            full_name='Resident A', national_id='1111111111', role='resident'
        )

        self.user_b = User.objects.create_user(
            phone='09122222222', password='passwordB',
            full_name='Resident B', national_id='2222222222', role='resident'
        )

        self.request_b = ServiceRequest.objects.create(
            title="Fix AC", description="It is blowing warm air.", resident=self.user_b
        )
    ...
    def test_logged_in_resident_can_create_request(self):
        ...
        payload = {"title": "Leaky Faucet", "description": "The kitchen sink is leaking."}
    ...
    def test_resident_fetching_requests_is_isolated(self):
        ...
        ServiceRequest.objects.create(
            title="Lightbulb out", description="Hallway light needs replacing.", resident=self.user_a
        )
        self.client.force_authenticate(user=self.user_a)
